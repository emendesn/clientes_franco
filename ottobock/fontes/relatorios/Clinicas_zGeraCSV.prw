...
STATIC PROCEDURE zzOkProc() 
	...
	_cLin := "Filial;KUNNR;Land1;Name1;Stras;PSTLZ;Ort01;Regio"
	...
	Fwrite(_cArq, _cLin + CHR(13) + CHR(10))             
		...
		_cSql := " SELECT"
		_cSql += "		D2_CLIENTE AS CLIENTE, D2_FILIAL AS FILIAL, A1_NOME, A1_END, A1_CEP, A1_MUN, A1_EST "
		_cSql += " FROM "
		...
		_cSql += " GROUP BY D2_FILIAL, D2_CLIENTE, A1_NOME, A1_END, A1_CEP, A1_MUN, A1_EST "	
		_cSql += " ORDER BY "	
		_cSql += " D2_FILIAL, D2_CLIENTE"	                       
	...
	_cLin := "Filial;VW;Kunde;Material;Bezeichnung;WG;Fakturadat;Nettoabsatz;Nettowert;Netto VW;currency"
	...
	Fwrite(_cArq, _cLin + CHR(13) + CHR(10))             
		...
		_cSql := " SELECT CSV.[CODIGO FILIAL] AS FILIAL,"
		...
		_cSql += " GROUP BY "	
		_cSql += "		CSV.[CODIGO FILIAL], CSV.CODCLI, CSV.OBSAP, ZAA.ZAA_UMEXT"
		_cSql += " ORDER BY "	
		_cSql += "		CSV.[CODIGO FILIAL], CSV.CODCLI, CSV.OBSAP"
		...
		if (cAlia2)->(Eof())
			...
			While !(cAlia2)->(Eof())
				...
				_cLin := AllTrim((cAlia2)->FILIAL) + ";"
				_cLin += iif(Posicione("SA1",1, xFilial("SA1")+AllTrim((cAlia2)->CODCLI),"A1_ZZTRANS")= "S","V03;","V04;")    
