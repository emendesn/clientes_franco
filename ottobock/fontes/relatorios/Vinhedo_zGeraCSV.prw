...
User Function zGeraCsv()
	...
	_cTitulo:= OemToAnsi("Geraçãoo do Arquivo Transaction")
	_cText1 := OemToAnsi("Esta rotina tem como objetivo Gerar o Transaction File")
	_cText2 := OemToAnsi("para envio de informações com Turnover Mensal.")
	...
	While .T.
		...
		If nOpc == 1
			RptStatus({||zzOkProc()},"Processando Geração de Arquivo...")
		EndIf
...
STATIC PROCEDURE zzOkProc() 
	...
	Fwrite(_cArq, _cLin + CHR(13) + CHR(10))             
		...
		_cSql += " FROM "
		_cSql += " 		"+RetSQLName("SD2")+" INNER JOIN "+RetSQLName("SA1")+" ON A1_FILIAL+A1_COD = D2_FILIAL+D2_CLIENTE "
		_cSql += " WHERE "
		...
		_cSql += " 		D2_EMISSAO LIKE '"+_cAno+_cMes+"%' AND SD2020.D_E_L_E_T_ <> '*' "
		_cSql += " ORDER BY "	
		_cSql += "	D2_CLIENTE"	                       
		...
		if (cAlias)->(Eof())
			msgAlert("Registro não encontrado para Gerar " + _cDir + "!!!","Atenção")
			_lGera := .F.
		else
...
STATIC PROCEDURE ValidPerg(cPerg)
	...
	PutSx1( cPerg,"01", "Mês ? ", "Mês ? ", "Month ? ", "mv_ch1","N",02,0,0,"C","","","","","mv_par01","","","","","","","","","","","","","","","","","","","","","","","","","",".ZZGCSV501.") 
	PutSx1( cPerg,"02", "Ano ? ", "Ano ? ", "Ano ?   ", "mv_ch2","N",04,0,0,"C","","","","","mv_par01","","","","","","","","","","","","","","","","","","","","","","","","","",".ZZGCSV502.")
	PutSx1( cPerg,"03", "Diretório ? ", "Directorio ? ", "Directory  ? ", "mv_ch2","C",99,0,0,"F","","","","","mv_par02","","","","","","","","","","","","","","","","","","","","","","","","","",".ZZGCSV503.")
PutSx1( cPerg,"04", "UN Pais ? ", "UN Pais ? ", "UN Pais ? ", "mv_ch3","C",03,0,0,"F","","","","","mv_par03","","","","","","","","","","","","","","","","","","","","","","","","","",".ZZGCSV504.")
	
	AADD(aHelpPor,"Mês de Referência para")
	AADD(aHelpPor,"geração do arquivo.")
	AADD(aHelpEng,"Mês de Referência para")
	AADD(aHelpEng,"geração do arquivo.")
	AADD(aHelpSpa,"Mês de Referência para") 
	AADD(aHelpSpa,"geração do arquivo.")
	...
	AADD(aHelpPor,"Ano de Referência para")
	AADD(aHelpPor,"geração do arquivo.")
	AADD(aHelpEng,"Ano de Referência para")
	AADD(aHelpEng,"geração do arquivo.")
	AADD(aHelpSpa,"Ano de Referência para") 
	AADD(aHelpSpa,"geração do arquivo.")
	...
	AADD(aHelpPor,"Diretório para Gravação do Arquivo.")
	AADD(aHelpEng,"Diretório para Gravação do Arquivo.")
	AADD(aHelpSpa,"Diretório para Gravação do Arquivo.") 
	PutSX1Help("P.ZZGCSV502.",aHelpPor,aHelpEng,aHelpSpa)
	aHelpPor := {}
	aHelpSpa := {}
	aHelpEng := {}
