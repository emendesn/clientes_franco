...
// Cria e abre o arquivo texto.
	...
	_cLocal1 := ALLTRIM(_cLocal) + "90059_CUST_" + _cMonth + "_" + strzero(val(substr(_cYear,3,2)),2)+ ".csv"
	...
	// Cria e abre o arquivo texto.
		_cLocal2 := ALLTRIM(_cLocal) + "90059_TRANS_" + _cMonth + "_" + strzero(val(substr(_cYear,3,2)),2)+ ".csv"
	...
	//	csql+= "GROUP BY CODCLI, OBSAP ORDER BY CODCLI, OBSAP"
		//TCQUERY csql
		_cSql:= "SELECT CSV.[CODIGO CLIENTE] AS CODCLI, MAX(CSV.[FECHA REDUZIDA]) AS DATA, CSV.[REFERENCIA SAP] AS OBSAP, MAX(CSV.DESCRIPCION) AS DESCR, MAX(CSV.[GRUPO DEL PRODUCTO]) AS GRUP, "
	...
	//	dbSelectArea("TRN")
		...
		MsgBox("Arquivo criado com sucesso !","Gera磯 de CSV","INFO")		
